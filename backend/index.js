...
const markAbsenteesForTest = async (testId) => {
    try {
        const students = await Student.find({}, 'testId scores alarm');
        ...
        try {
            ...
            if (status === 'completed' && !absenteeJobs.has(exam.testId)) {
                const absentJob = nodeSchedule.scheduleJob(new Date(Date.now() + 1800 * 1000), async () => {
                    await markAbsenteesForTest(exam.testId);
    ...
    (req, res, next) => {
        ...
        passport.authenticate('google', (err, user, info) => {
            ...
            try {
                ...
                const userStr = encodeURIComponent(JSON.stringify({ name: user.name, email: user.emailID, dp: user.profilePicture || '' }));
                const frontendUrl = process.env.FRONTEND_URL || 'https://zest-kohl-xi.vercel.app';
                const redirectUrl = `${frontendUrl}/home?token=${token}&user=${userStr}&remember=${!!remember}`;
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        res.json({ token, user: { name: student.name, email: student.emailID } });
...
app.get('/api/exams/by-testid/:testId', async (req, res) => {
    try {
        const exam = await Exam.findOne({ testId: req.params.testId });
...
app.get('/api/test-contents/:testId', async (req, res) => {
    try {
        const testContent = await TestContent.findOne({ testId: req.params.testId });
...
app.post('/api/test-contents', async (req, res) => {
    try {
        ...
        if (testContent) {
            ...
            testContent = new TestContent({ testId, questions });
...
app.post('/api/test/submit', async (req, res) => {
    try {
        ...
        const testContent = await TestContent.findOne({ testId });
        if (!testContent) return res.status(404).json({ message: 'Test content not found' });
        const exam = await Exam.findOne({ testId });
...
app.get('/api/student/submitted-tests', async (req, res) => {
    try {
        ...
        const student = await Student.findById(decoded.id, 'testId');
        if (!student) return res.status(404).json({ message: 'Student not found' });
...
app.get('/api/admin/notifications', async (req, res) => {
    try {
        const notifications = await Notification.find({ type: 'late_entry' }).sort({ createdAt: -1 });
...
app.get('/api/admin/reports/alarms', async (req, res) => {
    try {
        const students = await Student.find({}, 'name emailID testId scores alarm');
        const rows = [];
...
app.get('/api/student/analytics', async (req, res) => {
    try {
        ...
        const token = authHeader.split(' ')[1];
        const decoded = jwt.verify(token, process.env.JWT_SECRET || 'fallback_secret');
        const student = await Student.findById(decoded.id, 'name scores testId');
        if (!student) return res.status(404).json({ message: 'Student not found' });

        const exams = await Exam.find({}, 'testId examName totalMarks').lean();
...
app.get('/api/admin/attendance', async (req, res) => {
    try {
        const exams = await Exam.find({}, 'testId examName examDate').sort({ examDate: -1 }).lean();
        const students = await Student.find({}, 'name emailID testId scores').lean();
...
app.get('/api/leaderboard', async (req, res) => {
    try {
        const students = await Student.find({}, 'name scores profilePicture');
